...
* Are there any NA values? No **No NA Values**
* What differences do you notice between the structure of the two data frames? *The first one is much larger. Also second one just contains one state, MI. vs all states and it has a peninsula field*
...
ggplot(county_df) + geom_boxplot(aes(x=State, y=Income)) + 
                 theme(axis.title.x = element_text(face="bold", colour="steelblue", size=20),
           axis.text.x  = element_text(angle=90, vjust=0.5, size=10))
ggplot(county_df) + geom_density(aes(Income), fill="#FF9999", colour="black")
ggplot(county_df) + geom_violin(aes(x=State, y=Income)) + 
                 theme(axis.title.x = element_text(face="bold", colour="steelblue", size=20),
           axis.text.x  = element_text(angle=90, vjust=0.5, size=10))
```
